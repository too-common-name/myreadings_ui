<template>
  <v-container fluid class="pa-4">
    <v-row v-if="!display.xs.value">
      <v-col cols="12" md="3">
        ...
        <v-list nav>
          <v-list-item
            ...
            :key="collection.readingListId"
            @click="handleCollectionClick(collection)"
            :active="selectedCollection?.readingListId === collection.readingListId"
            link
          >
      ...
      <v-col cols="12" md="9">
        ...
        <v-data-table
          ...
          :items="booksInCollection"
          item-value="bookId"
          class="elevation-0"
          :loading="loadingBooks"
          no-data-text="No books in this collection."
        >
          <template v-slot:item.cover="{ item }">
            <v-img
              :src="getCoverUrl(item, 'M')"
              :alt="item.title"
              width="50"
              height="75"
              aspect-ratio="2/3"
          ...
          <template v-slot:item.averageRating="{ item }">
            <v-rating
              :model-value="item.reviewStats?.averageRating ?? 0"
              length="5"
              size="18"
            ...
            <span class="text-body-2 ms-1"
              >({{ item.reviewStats?.averageRating?.toFixed(1) || 'N/A' }})</span
            >
          </template>
          <template v-slot:item.userRating="{ item }">
            <v-rating
              :model-value="item.userRating ?? 0"
              length="5"
              size="18"
    ...
    <CreateReadingListDialog
      :model-value="showCreateListDialog"
      @update:model-value="showCreateListDialog = $event"
