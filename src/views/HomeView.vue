<template>
  <div class="new-releases">
    ...
    <swiper-container :breakpoints="swiperBreakpoints" speed="500" loop>
      <swiper-slide v-for="book in recentBooks" :key="book.bookId">
        <BookCard :genre="book.genre" :coverUrl="getCoverUrl(book, 'L')" :title="book.title"
          :description="book.description" @bookCardClicked="bookCardClicked(book)" />
      </swiper-slide>
    </swiper-container>
  </div>
  ...
  <BookDialog v-if="selectedBookForDetails" :model-value="showBookDetailsDialog"
    @update:model-value="showBookDetailsDialog = $event" :book="selectedBookForDetails" :select-options="readingLists"
    :review-stats="selectedBookForDetails?.reviewStats" :user-rating="userRatingForSelectedBook"
    :book-reading-list-id="bookReadingListIdForSelectedBook" @confirm-changes="handleConfirmChanges" />
</template>
